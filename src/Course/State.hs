...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  f <$> (State run) = State (\s -> first f (run s))
...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s a
    -> State s b
  f =<< (State runx) = State (\s -> let (x, s') = runx s in runState (f x) s')
...
findM ::
  ...
  -> List a
  -> m (Optional a)
findM p (x :. xs) = (\q -> if q then pure (Full x) else findM p xs) =<< p x
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat xs = fst $ (runState (findM (\x -> State (\s -> (S.member x s, S.insert x s))) xs)) S.empty
