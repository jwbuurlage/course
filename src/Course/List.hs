...
seqOptional = foldRight maybeConcat (Full Nil)
  where maybeConcat :: Optional a -> Optional (List a) -> Optional (List a)
        maybeConcat (Full x) (Full xs) = Full (x :. xs)
        maybeConcat _ _ = Empty
